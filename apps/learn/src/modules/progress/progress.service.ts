...
const LEVEL_ORDER = ['NEW', 'LEVEL_1', 'LEVEL_2', 'LEVEL_3', 'LEVEL_4', 'LEVEL_5'] as const;
...
export class ProgressService {
  ...
  async getMyCourses(userId: string, filter: CourseProgressFilterDto) {
    ...
    const findManyArgs: Prisma.UserCourseProgressFindManyArgs = {
      ...
      include: {
        course: {
          select: { id: true, title: true, enTitle: true, icon: true, image: true },
        },
      },
    };
  ...
  async getMyLessons(userId: string, filter: LessonProgressFilterDto) {
    ...
    const findManyArgs: Prisma.UserLessonProgressFindManyArgs = {
      ...
      include: {
        lesson: {
          select: { id: true, title: true, order: true, courseId: true, image: true },
        },
      },
    };
  ...
  async getProgressStats(userId: string) {
    const [totalWords, masteredWords, dueForReview, courseProgress, lessonProgress] =
      await Promise.all([
        ...
        this.prisma.userWordProgress.count({
          where: {
            userId,
            OR: [{ nextReview: { lte: new Date() } }, { nextReview: null, status: 'NEW' }],
          },
        }),
