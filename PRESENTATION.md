...
*   **Giới hạn giải pháp cũ:**
    *   *Leitner (Flashcard truyền thống):* Phân chia hộp thẻ cố định, thiếu linh hoạt, dễ gây nản lòng hoặc quá tải.
    *   *SuperMemo-2 (SM-2):* Lập lịch dựa trên quy luật chung của số đông, không thích ứng theo hành vi và tốc độ học của từng cá nhân ("Ease Hell" - kẹt trong vòng lặp ôn tập từ khó).
...
## Slide 3. TỔNG QUAN GIẢI PHÁP & HỆ SINH THÁI ĐỀ XUẤT

### Nội dung Slide
*   **Sự kết hợp giữa 3 trụ cột kỹ thuật:**
    1.  **Thuật toán FSRS v6 (Free Spaced Repetition Scheduler):** Dự đoán xác suất nhớ lại (Retrievability) để sinh lịch ôn tập tối ưu cá nhân hóa.
    2.  **Generative AI (Local LLM + Cloud API):** Sử dụng Ollama (Local LLM) để sinh bài tập, hội thoại ngữ cảnh miễn phí và ElevenLabs (Cloud API) để xử lý âm thanh chất lượng cao.
    3.  **Kiến trúc Microservices:** Tách biệt các miền nghiệp vụ (Domain-driven), đảm bảo tính cô lập dữ liệu, khả năng chịu tải và mở rộng độc lập.

### Kịch bản thuyết trình
> Để giải quyết triệt để các vấn đề trên, em đề xuất giải pháp tích hợp gồm ba trụ cột kỹ thuật chính:
>
> Thứ nhất là **Thuật toán FSRS v6** - thuật toán lặp khoảng cách tiên tiến nhất hiện nay, giúp mô hình hóa trạng thái bộ não của từng người dùng để dự đoán chính xác xác suất nhớ lại từ vựng tại thời điểm bất kỳ.
>
> Thứ hai là **Generative AI** - thiết lập một kiến trúc lai (Hybrid AI). Hệ thống sử dụng **Ollama** để chạy mô hình ngôn ngữ lớn cục bộ (Local LLM) giúp sinh bài tập ngữ cảnh và đóng vai gia sư ảo miễn phí, kết hợp với **ElevenLabs API** để mang lại chất lượng giọng nói tự nhiên nhất.
>
> Thứ ba là **Kiến trúc Microservices** - toàn bộ hệ thống được chia nhỏ thành các dịch vụ độc lập, giúp xử lý các tác vụ nặng (như tính toán FSRS hay sinh AI) mà không làm ảnh hưởng đến luồng nghiệp vụ cốt lõi của người dùng.

---

## Slide 4. KIẾN TRÚC HỆ THỐNG MICROSERVICES & DATABASE ISOLATION
...
## Slide 5. API GATEWAY - TRỤ CỘT BẢO MẬT & ĐIỀU PHỐI
...
## Slide 6. LEARN SERVICE CACHING & INVALIDATION STRATEGY
...
*   **Mô hình nhận thức DHP (Difficulty - Stability - Retrievability):**
    ...
    *   **Retrievability ($R$):** Xác suất nhớ lại của từ vựng tại thời điểm kiểm tra:
        ...
        *(Với FSRS v4.5/v6, $\text{FACTOR} = \frac{19}{81}$, $\text{DECAY} = -0.5$)*
...
*   **Quy trình huấn luyện & Kiểm soát chất lượng (Evaluation Gate):**
    *   *Điều kiện huấn luyện:* Chỉ tự động chạy huấn luyện lại bộ trọng số (21 tham số weights) khi người dùng tích lũy đủ dữ liệu (`valid_logs >= 20`) và cách lần train trước tối thiểu 3 ngày.
    *   *Cổng kiểm duyệt chất lượng:* So sánh mô hình mới (Candidate weights) và mô hình cũ (Baseline weights) thông qua độ đo **Log-Loss** (hoặc Calibration Error). Chỉ chấp nhận cập nhật nếu độ lỗi giảm tối thiểu 2% (`FSRS_TRAIN_MIN_IMPROVEMENT_PCT = 0.02`).
    *   *Rollback:* Cho phép phục hồi về phiên bản trọng số ổn định trước đó nếu phát hiện hành vi học bất thường phá vỡ mô hình học máy.
...
*   **Tối ưu hóa Pipeline đa phương tiện (Frontend & Backend):**
    ...
    *   **3D Avatar Rendering Optimization:**
        ...
        *   *Kết quả thực nghiệm:* VRAM giảm từ **980MB xuống còn 210MB**, loại bỏ hoàn toàn hiện tượng giật khung hình khi chuyển cảnh trên các thiết bị cấu hình thấp.
...
>
> Cuối cùng, kết quả học tập sẽ được hiển thị trên Dashboard. Các API báo cáo của chúng ta không chỉ trả về số liệu thô mà còn trả về cả các đoạn nhận xét bằng tiếng Việt được sinh tự động, ví dụ như: *"Ngày mai có 28 từ đến hạn, bạn nên ôn tập thành 2 phiên ngắn để tránh dồn ứ bài học"*.
